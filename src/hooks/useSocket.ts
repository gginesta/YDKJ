...
/**
 * Register all socket event handlers ONCE globally.
 * This prevents the bug where component unmounts call socket.off()
 * and remove handlers that other components still need.
 */
function ensureHandlersRegistered(socket: AppSocket): void {
  if (handlersRegistered) return;
  handlersRegistered = true;

  console.log('[Socket] Registering global event handlers');
  ...
  socket.on('game_over', ({ finalScores, hostScript, audioUrl }) => {
    ...
    if (audioUrl) playAudio(audioUrl);
    else if (hostScript) setTimeout(() => speakText(hostScript), 1500);
  });
...
export function useSocket() {
  ...
  useEffect(() => {
    ...
    // Register handlers once globally — never removed on component unmount
    ensureHandlersRegistered(socket);
    ...
    // No cleanup — handlers are global and must persist across component mounts/unmounts
  }, []);
